...
//KP changed due to square pop up and change dimention
...
- (id)initWithTitle:(NSString *)title message:(NSString *)message {
	...
	if (self)
    {
        ...
        self.titleFont = [UIFont fontWithName:CGRegular size:textSize];
  ...
  otherButtonTitles:(NSString *)otherButtonTitles, ...
{
	...
	if (self) {
		if (cancelButtonTitle) {
            ...
            self.cancelButton.titleLabel.font = [UIFont fontWithName:CGRegular size:15];
		}
...
- (void)initialize {
//	_backgroundColor = [UIColor colorWithWhite:0.22 alpha:1.0];//KP
    _backgroundColor = [UIColor whiteColor];

	_backgroundGradation = 0.1;
//	_strokeColor = [UIColor colorWithWhite:0.8 alpha:1.000];//KP
    _strokeColor = [UIColor whiteColor];

//	_strokeWidth = 3.0;
//	_cornerRadius = 6.0;//KP
    
    _strokeWidth = 0.0;//KP
    _cornerRadius = 0.0;//KP
	
	_titleFont = [UIFont boldSystemFontOfSize:15.0];
//	_titleColor = [UIColor whiteColor];//KP
    _titleColor = [UIColor darkGrayColor];//KP
//    _titleColor = [UIColor blackColor];
//    _titleColor = [APP_DELEGATE colorWithHexString:yellow_color];
//    _titleColor = [UIColor clearColor];


//	_titleShadowColor = [UIColor blackColor];//KP
    _titleShadowColor = [UIColor clearColor];

	_titleShadowOffset = CGSizeMake(0.0, -1.0);
	
	_messageFont = [UIFont systemFontOfSize:14.0];
//	_messageColor = [UIColor whiteColor];//KP
//    _messageColor = [UIColor darkGrayColor];//KP
    _messageColor = [UIColor blackColor];


//	_messageShadowColor = [UIColor blackColor];//KP
    _messageShadowColor = [UIColor clearColor];

	_messageShadowOffset = CGSizeMake(0.0, -1.0);
	
//	_buttonBackgroundColor = [UIColor colorWithWhite:0.35 alpha:1.0];//KP
//    _buttonBackgroundColor = [UIColor blueColor];
//    _buttonBackgroundColor = [APP_DELEGATE colorWithHexString:yellow_color];//KP
...
//	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(deviceOrientationChanged:) name:UIDeviceOrientationDidChangeNotification object:nil];//KP
	
	// register for keyboard notifications
//	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillShow:) name:UIKeyboardWillShowNotification object:nil];//KP
//	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillHide:) name:UIKeyboardWillHideNotification object:nil];//KP
...
//	button.titleLabel.font = [UIFont boldSystemFontOfSize:16.0];//KP
    button.titleLabel.font = [UIFont boldSystemFontOfSize:18.0];
//    button.titleLabel.font = [UIFont fontWithName:SinkinSansBoldFont size:16];//KP change button text color

//    [button setTitleColor:[APP_DELEGATE colorWithHexString:yellow_color] forState:UIControlStateNormal];
    [button setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
    button.titleLabel.font = [UIFont fontWithName:CGRegular size:15];
...
//                self.transform = translate;//KP
    ...
    else if (animation == URBAlertAnimationTopToBottom)//KP
    {
        if (show)
        {
            [self showOverlay:YES];
...
//            CGAffineTransform rotate = CGAffineTransformRotate(transform, 50.0 * M_PI / 180.0);//KP
//            CGAffineTransform translate = CGAffineTransformTranslate(transform, 00.0, -screenSize.height / 2.0 - CGRectGetWidth(self.bounds));//top to bottom
            CGAffineTransform translate1 = CGAffineTransformMakeTranslation(0, 568);//KP bottom to top

//            self.transform = translate;
            self.transform = translate1;//KP
...
//    [self.contentView addSubview:beloved]; //temp 16-4 //KP
...
//    [self.contentView addSubview:beloved];
	...
	if (self.title.length > 0) {
		if ([self.title respondsToSelector:@selector(boundingRectWithSize:options:attributes:context:)]) {
			CGRect titleBounds = [self.title boundingRectWithSize:CGSizeMake(CGRectGetWidth(textFrame), MAXFLOAT) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:@{NSFontAttributeName:[UIFont fontWithName:CGRegular size:textSize]}
														  context:nil];
			titleHeight = titleBounds.size.height;
		}
		else
        {
			CGSize titleSize = [self.title sizeWithFont:[UIFont fontWithName:CGRegular size:textSize] constrainedToSize:CGSizeMake(CGRectGetWidth(textFrame), MAXFLOAT) lineBreakMode:NSLineBreakByWordWrapping];
...
//	CGContextSetLineWidth(context, 1.0f);//KP
    CGContextSetLineWidth(context, 0.0f);//KP 17-4
...
//	keylinePath.lineWidth = 1.0;//KP remove seperator line
    keylinePath.lineWidth = 0.0;
//    keylinePath.lineWidth = 0.5;

	
//	CGContextTranslateCTM(context, 0, CGRectGetMinY(hatchFrame) - 2.0);//KP
    CGContextTranslateCTM(context, 0, CGRectGetMinY(hatchFrame) + 7.0);

	[self.darkKeylineColor setStroke];
	[keylinePath stroke];
//	CGContextTranslateCTM(context, 0, 1.0);//KP
	...
	[self drawText:self.title inRect:layout.titleRect font:[UIFont fontWithName:CGRegular size:textSize] color:self.titleColor shadowColor:self.titleShadowColor shadowOffset:self.titleShadowOffset];
...
//	UIColor *gradientInner = [UIColor colorWithRed: 0 green: 0 blue: 0 alpha: 0.1];
    ...
    UIColor *gradientInner = [UIColor colorWithRed: 0 green: 0 blue: 0 alpha: 0.5];//KP for removing round background shadow
...
//		_strokeColor = [UIColor colorWithWhite:0.2 alpha:1.0];//KP
        _strokeColor = [UIColor whiteColor];
...
//    UIColor *buttonInnerGradientColor2 = [UIColor colorWithRed:255.0/255 green:213.0/255 blue:132.0/255 alpha:1];
    ...
    UIColor *buttonInnerGradientColor2 = [UIColor clearColor];//KP
...
//								0);
//	CGContextRestoreGState(context);//KP
	
	// outer drawing
//	CGRect buttonOuterRect = CGRectInset(frame, 4.0, 4.0);
    CGRect buttonOuterRect = CGRectInset(frame, 0.0, 0.0);//KP
	...
	// inner drawing
//	CGRect buttonInnerRect = CGRectInset(frame, 6.0, 6.0);//KP
...
//	UIColor *buttonInnerGradientColor2 = [self.backgroundColor adjustBrightness:1.05];
    ...
    UIColor *buttonInnerGradientColor2 = [UIColor lightGrayColor];//KP
    
//    UIColor *buttonInnerGradientColor = [APP_DELEGATE colorWithHexString:@"d3d3d3"];//light gray color
//    UIColor *buttonInnerGradientColor2 = [APP_DELEGATE colorWithHexString:@"d3d3d3"];//KP
