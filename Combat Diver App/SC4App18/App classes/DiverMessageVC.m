...
@interface DiverMessageVC ()<FCAlertViewDelegate>
{
    NSTimer *  msgTimer;
...
- (void)viewDidLoad
{
    ...
    [lblTitle setText:@"Diver Messages"];
    ...
    lblHint.text = NSLocalizedString(@"Sync Diver messages to device by tapping on SYNC button.",@"");
    ...
    if (IS_IPHONE)
    {
        ...
        lblHint.text = NSLocalizedString(@"Sync Diver messages to device",@"");
...
-(void)setupMainContentView:(int)headerHeights
{
    ...
    //    UILabel * lblBtn  = [[UILabel alloc] init];
    //    lblBtn.frame = CGRectMake(0, DEVICE_HEIGHT-60, DEVICE_WIDTH, 60);
    //    lblBtn.backgroundColor = [UIColor blackColor];
    //    lblBack.alpha = 0.4;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [alert showAlertInView:self
              ...
              withSubtitle:@"Update Diver Message"
           withCustomImage:nil
...
-(void)btnAddClick
{
    ...
    UITextField *customField = [[UITextField alloc] init];
    customField.placeholder = @"Enter Diver Message";
    [alert addTextFieldWithCustomTextField:customField andPlaceholder:nil andTextReturnBlock:^(NSString *text) {
        NSLog(@"Custom TextField Returns: %@", text); // Do what you'd like with the text returned from the field
    ...
    [alert showAlertInView:self
                 withTitle:@"Combat Diver"
              withSubtitle:@"Enter Diver Message"
           withCustomImage:nil
       withDoneButtonTitle:nil
                andButtons:nil];
}
    ...
    else
    {
        if ([text length] <=18)
        {
            if (isEdited)
            {
                ...
                NSString * strInsertCan = [NSString stringWithFormat:@"update DiverMessage set Message ='%@' where indexStr = '%@'",text,updateIndx];
            ...
            else
            {
                ...
                NSString * strInsertCan  = [NSString stringWithFormat:@"insert into 'DiverMessage' ('Message','is_emergency','indexStr') values ('%@','No','%@')",text,[NSString stringWithFormat:@"%d",myInt+1]];
...
-(void)sendCandMsg
{
    if (kpCount >= [arrMessages count])
    {
        ...
        [alert showAlertInView:self
                     withTitle:@"Combat Diver"
                  withSubtitle:@"Diver messages synced successfully."
               withCustomImage:nil
           withDoneButtonTitle:nil
                    andButtons:nil];
    }
